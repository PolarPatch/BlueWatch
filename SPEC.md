...
fork — same "detach and own it" pattern already used for claude-buddy-cyd
earlier in this project. Not on GitHub yet, staying local until it's further
along.
...
(Reference only — the running instance is on raspi3-bluehood, 192.168.1.72 /
.186, not necessarily the base for BlueWatch's own codebase, but useful
...
1. Does the 24h temporary notification override reset on a fixed clock, or
   expire after the first arrive/depart event fires? (leaning: fixed 24h
   clock, survives multiple events — confirm)
2. How should randomized-MAC devices be correlated across rotations so
   category/watch assignments don't silently fall off? (service UUIDs +
   manufacturer data + advertised name matching is the standard approach;
   needs its own design pass)
3. Tech stack: continue with Python/bleak/aiosqlite/aiohttp (matching
   bluehood, fastest path to reusing working scan/persistence code), or
   something else? Not discussed yet — assumed Python continuation unless
   corrected.
4. Where does BlueWatch actually run — same raspi3-bluehood device (replacing
   bluehood there), a different Pi, or developed on the Mac first and
   deployed later? Not discussed yet.

## Status

2026-09-09: design conversation in progress, nothing built yet. bluehood is
still running on raspi3-bluehood (192.168.1.72 / .186) with **ntfy
notifications disabled** (`ntfy_enabled: false`, topic cleared) while
BlueWatch is being designed, to stop notification spam from randomized-MAC
re-triggers during this phase. Topic to restore later: `wifiproject94-909c7e53`.
